...
async function main() {
    ...
    const posts = await fetchJson('/api/blog/posts?limit=1&page=1');
    ...
    const single = await fetchJson(`/api/blog/posts/${encodeURIComponent(first.slug)}`);
    assert(single.data?.slug === first.slug, 'single-post slug mismatch');

    const content = await fetchJson(`/api/blog/posts/${encodeURIComponent(first.slug)}/content`);
