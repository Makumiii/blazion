...
export interface BlogPackStoredPost extends BlogPost {
    notionUrl: string;
}

export interface BlogPackPostFacets {
    authors: Array<{ value: string; count: number }>;
    segments: Array<{ value: string; count: number }>;
}

export interface BlogPackListReadyPostsOptions {
    page: number;
    limit: number;
    q: string;
    dateFrom: string;
...
export interface BlogPackDatabase {
    listReadyPosts: (
        options: BlogPackListReadyPostsOptions,
    ) => { data: BlogPackStoredPost[]; total: number; facets: BlogPackPostFacets };
    listAllReadyPosts: () => BlogPackStoredPost[];
    getReadyPostBySlug: (slug: string) => BlogPackStoredPost | null;
}

export interface BlogPackNotionService {
    getRecordMap: (notionPageId: string) => Promise<unknown>;
    getBlockContent: (notionPageId: string) => Promise<unknown[]>;
}
            ...
            return c.json(
                {
                    ...
                    message:
                        'Set NOTION_API_KEY and run setup to link a Notion database for the blog pack.',
                },
                503,
            );
        }
...
export async function setupBlogPackDatabase(input: {
    client: Client;
    pageId: string;
}): Promise<string> {
    const database = await input.client.databases.create({
        parent: {
            type: 'page_id',
    ...
    try {
        await input.client.databases.update({
...
function rankRecommendations(input: {
    current: BlogPackStoredPost;
    candidates: BlogPackStoredPost[];
    nowMs: number;
    limit: number;
    options: BlogPackRouteOptions;
}): { posts: BlogPackStoredPost[]; strategy: 'related_ids' | 'tags' | 'segment' | 'featured' | 'latest' } {
...
function resolvePostTimestampMs(post: BlogPackStoredPost): number {
